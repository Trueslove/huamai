<template>
  <view>
    ...
    <view class="info_list_wrap">
      ...
      <view class="info_box">
        <view class="info_pa" >提现 
            <text v-if="item.shzt" class="text-sm margin-left-xs on">已转账</text>
            <text v-if="!item.shzt"  class="text-sm  margin-left-xs off">待转账</text>
            </view>
...
export default {
  ...
  methods: {
    getData() {
      this.$api
        .get(global.apiUrls.getTxjl, { pageNum:this.pageNum,pageSize:10})
        .then((result) => {
          console.log(111)
