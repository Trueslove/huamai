<template>
	<view class="mo-navbar" :style="[{height: CustomBar+'px',paddingTop: StatusBar+'px',color: color,background:bg,paddingRight:PR +'px',top:top + 'px'}]">
		<view  class="flex align-center mo-content" :style="[{height: CustomBar-StatusBar+'px'}]">
			<view class="mo-back text-xl text-lg flex align-center" v-if="isBack" @tap="back">
				<text class="cuIcon-back"></text>
				<text class="text-lg" v-if="isBackText">{{i18n['返回']}}</text>
			</view>
			<view class="cu-bar search mo-search" :style="[{minHeight: CustomBar-StatusBar+'px'}]" v-if="isSearch">
		...
		<view class="mo-title text-lg text-center text-cut" :style="[{height: CustomBar+'px',paddingTop: StatusBar+'px',lineHeight:CustomBar-StatusBar+'px'}]" v-if="!isSearch">
			{{title}}
		</view>
	</view>
</template>
