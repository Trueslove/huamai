<template>
  <view class="mo-notwork" v-if="!isConnected">
    <view class="mo-count-df" v-if="mode == 'full'" :style="{ zIndex: zIndex }">
      <image class="icon" :src="image" mode="aspectFit"></image>
      <view class="tips">{{ tips }}</view>
      <view class="text-gray text-sm margin-bottom-sm">
        请检查网络，或者前往
        <text class="text-blue">设置</text>
      </view>
      <view @click="getNetworkStattus">点击刷新</view>
    </view>
    <view class="mo-count-tips" v-if="mode == 'tips'">
      <text></text>
      <text>{{ tips }}</text>
    </view>
  </view>
